...
class window.AmericanRoulette.View extends App.MasterView

  localStoragePath: "satoshibet.american_roulette.game_state"
  ...
  renderLuckyNumber: (luckyNumber)->
    @tpl = "american-roulette-lucky-number-tpl"
  ...
  renderWinLostAmount: (wonAmount, lostAmount)->
    @tpl = "american-roulette-game-result-tpl"
  ...
  getNumberColor: (number)->
    return "green" if number is "0" or number is "00"
  ...
  getZeroNumber: ()->
    @$zeroNumber = @getNumberElements(true).filter("#num-0")  if not @$zeroNumber
    $(@$zeroNumber)

  getDoubleZeroNumber: ()->
    @$doubleZeroNumber = @getNumberElements(true).filter("#num-00")  if not @$doubleZeroNumber
    $(@$doubleZeroNumber)
  ...
  getCornerElements: (cornerNumber)->
    ...
    # green numbers will be added later
    validNumbers = [2, 3] if cornerNumber is -1
    validNumbers = [1, 2] if cornerNumber is -2
    validNumbers = [2] if cornerNumber is 0
    cb = (el)->
      number = parseInt $(el).text()
  ...
  getSplitVElements: (splitNumber)->
    cb = (el)->
      number = parseInt $(el).text()
      (number is splitNumber) and (splitNumber isnt 0) or number is (splitNumber + 3)
    @getCachedElements "splitV", splitNumber, cb, true

  getSplitHElements: (splitNumber)->
    validNumbers = [splitNumber, splitNumber + 1]
  ...
  highlightCorner: ($el)->
    ...
    @getZeroNumber().addClass("highlight")  if cornerNumber is -2 or cornerNumber is 0
    @getDoubleZeroNumber().addClass("highlight")  if cornerNumber is -1 or cornerNumber is 0

  highlightSplitV: ($el)->
    splitNumber = parseInt $el.data("bet").replace("splitv_", "")
    @getSplitVElements(splitNumber).addClass("highlight")
    @getZeroNumber().addClass("highlight") if splitNumber is -2
    @getDoubleZeroNumber().addClass("highlight") if splitNumber is 0

  highlightSplitH: ($el)->
    splitNumber = parseInt $el.data("bet").replace("splith_", "")
    @getSplitHElements(splitNumber).addClass("highlight")
    if splitNumber is -2
      @getZeroNumber().addClass("highlight")
      @getDoubleZeroNumber().addClass("highlight")
